...
const SESSION_SECRET = process.env.SESSION_SECRET || 'angelcord-troque-esse-segredo';
...
// ===== ESTADO EM MEMÓRIA =====
// servers: Map<id, {id, name, isPrivate, password, ownerName, channels: [{id,name,type}], memberNames:Set}>
const servers = new Map();
// mensagens por canal: Map<channelId, [{user, text, time}]>
const channelMessages = new Map();
// membros conectados na chamada de voz por canal: Map<channelId, Set<socketId>>
...
io.on('connection', (socket) => {
  ...
  socket.on('join-voice', (channelId) => {
    ...
    // avisa quem já está na sala sobre o novo peer
    const existing = Array.from(room);
    socket.join('voice-' + channelId);
    room.add(socket.id);

    socket.emit('voice-existing-peers', existing.map(id => ({
      socketId: id,
      username: socketUsers.get(id) || '???'
    })));
  ...
  socket.on('screen-share-status', ({ channelId, sharing }) => {
    socket.to('voice-' + channelId).emit('peer-screen-share', { socketId: socket.id, sharing });
  });
